...
if [ -e unit1exercise2.tgz ]; then rm -f unit1exercise2.tgz; fi
# 1. Run script
bash $1
# 2. Get contents of tar ball, and sort them, and exclude time stamps
tar ztvf unit1exercise2.tgz | cut -c1-30,48- | LC_COLLATE=C sort > exercise2.txt
...
sizepoints=0
size=`cat $1 | wc -c`
if [ $size -le 2331 ]; then sizepoints=5; fi
if [ $size -le 1763 ]; then sizepoints=15; fi
if [ $size -le 1195 ]; then sizepoints=25; fi
if [ $size -le 1015 ]; then sizepoints=40; fi
