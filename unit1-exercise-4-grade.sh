...
tar ztvf unit1exercise4.tgz | cut -c1-30,48- | LC_COLLATE=C sort > exercise4.txt
...
sizepoints=0
size=`cat $1 | wc -c`
if [ $size -le 2461 ]; then sizepoints=5; fi
if [ $size -le 1802 ]; then sizepoints=15; fi
if [ $size -le 1144 ]; then sizepoints=25; fi
if [ $size -le 972 ]; then sizepoints=40; fi
