...
if [ -e unit1exercise3.tgz ]; then rm -f unit1exercise3.tgz; fi
# 1. Run script
bash $1
...
sizepoints=0
size=`cat $1 | wc -c`
if [ $size -le 2392 ]; then sizepoints=5; fi
if [ $size -le 1796 ]; then sizepoints=15; fi
if [ $size -le 1201 ]; then sizepoints=25; fi
if [ $size -le 1020 ]; then sizepoints=40; fi
